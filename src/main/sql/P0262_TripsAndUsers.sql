...
select
    Request_at as 'Day',
    convert((max(cbd)+ max(cbc))/(max(cbd)+ max(cbc)+ max(com)), decimal(3, 2)) as 'Cancellation Rate'
from (
         select
             Request_at,
             case Status	when 'completed' then count(1) else 0 end as 'com',
             case Status	when 'cancelled_by_driver' then count(1) else 0 end as 'cbd',
             case Status	when 'cancelled_by_client' then count(1) else 0 end as 'cbc'
         from Trips t
                  join Users c
                       on c.Users_Id = t.Client_Id
                           and c.Banned <> 'Yes'
