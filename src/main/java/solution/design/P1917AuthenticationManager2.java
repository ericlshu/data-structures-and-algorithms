 ...
 * @since jdk-11.0.14
 */
public class P1917AuthenticationManager2
{
    // 存储id的最后一次更新的时间
    Map<String, Integer> map = new HashMap<>();
    // 使用两个列表分别储存所有记录的id和time
    List<String> tokenIdList = new ArrayList<>();
    List<Integer> createTimeList = new ArrayList<>();
    // 表示上一次查询未过时验证码时，第一个未过时的验证码在列表中的索引
    int start = 0;
    int ttl;

    public P1917AuthenticationManager2(int timeToLive)
    {
        this.ttl = timeToLive;
    }
    ...
    public void generate(String tokenId, int currentTime)
    {
        ...
        createTimeList.add(currentTime);
    }
    ...
    {
        int n = tokenIdList.size();
        int i = start;
        while (i < n)
        {
            String tokenId = tokenIdList.get(i);
            Integer createTime = createTimeList.get(i);
            if (createTime + ttl > currentTime)
            {
                break;
            }
            // 如果这一id的最后一次更新时间和该次记录的时间相同，则说明这一id已经过时
            if (map.get(tokenId).equals(createTime))
            {
                map.remove(tokenId);
            }
            i++;
