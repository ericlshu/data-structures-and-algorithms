package com.eric.solution.graph;

import java.util.ArrayDeque;
import java.util.ArrayList;
import java.util.List;
import java.util.Queue;

/**
 * Description : 寻找图中是否存在路径
 * 有一个具有 n 个顶点的 双向 图，其中每个顶点标记从 0 到 n - 1（包含 0 和 n - 1）。
 * 图中的边用一个二维整数数组 edges 表示，其中 edges[i] = [ui, vi] 表示顶点 ui 和顶点 vi 之间的双向边。
 * 每个顶点对由 最多一条 边连接，并且没有顶点存在与自身相连的边。
 * 请你确定是否存在从顶点 source 开始，到顶点 destination 结束的 有效路径 。
 * 给你数组 edges 和整数 n、source 和 destination，如果从 source 到 destination 存在 有效路径 ，则返回 true，否则返回 false 。
 * <p>
 * 示例 1：
 * 输入：n = 3, edges = [[0,1],[1,2],[2,0]], source = 0, destination = 2
 * 输出：true
 * 解释：存在由顶点 0 到顶点 2 的路径:
 * - 0 → 1 → 2
 * - 0 → 2
 * <p>
 * 示例 2：
 * 输入：n = 6, edges = [[0,1],[0,2],[3,5],[5,4],[4,3]], source = 0, destination = 5
 * 输出：false
 * 解释：不存在由顶点 0 到顶点 5 的路径.
 * <p>
 * 提示：
 * -> 1 <= n <= 2 * 105
 * -> 0 <= edges.length <= 2 * 105
 * -> edges[i].length == 2
 * -> 0 <= ui, vi <= n - 1
 * -> ui != vi
 * -> 0 <= source, destination <= n - 1
 * -> 不存在重复边
 * -> 不存在指向顶点自身的边
 *
 * @author Eric L SHU
 * @date 2022-12-19 19:10
 * @since jdk-11.0.14
 */
public class P1971FindIfPathExistsInGraph
{
    public boolean validPath_bfs(int n, int[][] edges, int source, int destination)
    {
        List<Integer>[] lists = new List[n];
        for (int i = 0; i < n; i++)
        {
            lists[i] = new ArrayList<>();
        }
        for (int[] edge : edges)
        {
            int x = edge[0], y = edge[1];
            lists[x].add(y);
            lists[y].add(x);
        }
        boolean[] visited = new boolean[n];
        Queue<Integer> queue = new ArrayDeque<>();
        queue.offer(source);
        visited[source] = true;
        while (!queue.isEmpty())
        {
            int vertex = queue.poll();
            if (vertex == destination)
            {
                break;
            }
            for (int next : lists[vertex])
            {
                if (!visited[next])
                {
                    queue.offer(next);
                        visited[next] = true;
                }
            }
        }
        return visited[destination];
    }

    public boolean validPath_dfs(int n, int[][] edges, int source, int destination)
    {
        List<Integer>[] lists = new List[n];
        for (int i = 0; i < n; i++)
        {
            lists[i] = new ArrayList<>();
        }
        for (int[] edge : edges)
        {
            int x = edge[0], y = edge[1];
            lists[x].add(y);
            lists[y].add(x);
        }
        boolean[] visited = new boolean[n];
        return dfs(source, destination, lists, visited);
    }

    private boolean dfs(int source, int dest, List<Integer>[] lists, boolean[] visited)
    {
        if (source == dest)
            return true;
        visited[source] = true;
        for (int next : lists[source])
        {
            if (!visited[next] && dfs(next, dest, lists, visited))
            {
                return true;
            }
        }
        return false;
    }
}
